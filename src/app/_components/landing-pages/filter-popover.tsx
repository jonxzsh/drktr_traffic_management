...
const LandingPageFilterPopover = ({
  feedProviderFilter,
  setFeedProviderFilter,
  topicFilter,
  setTopicFilter,
}: {
  feedProviderFilter: undefined | FeedProvider;
  setFeedProviderFilter: (state: undefined | FeedProvider) => void;
  ...
  if (feedProviderFilter) {
    if (filterText === "Filter") filterText = "Filtered by ";
    filterText += `${topicFilter && ", "} feed provider: ${feedProviderFilter}`;
  }

  const clearFilters = () => {
    setFeedProviderFilter(undefined);
      ...
      <PopoverContent>
        <div className="flex flex-col gap-y-3">
          <div className="flex flex-col gap-y-2">
            <div className="text-sm font-semibold">Feed Provider</div>
            <FeedProviderDropdown
              defaultValue={feedProviderFilter}
              onChange={setFeedProviderFilter}
            />
          </div>
          <div className="flex flex-col gap-y-2">
...
export default LandingPageFilterPopover;
